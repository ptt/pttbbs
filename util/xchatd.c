...
exit_room(ChatUser *user, int mode, char *msg)
{
	...
	switch (mode)
	{
		...
		sprintf(chatbuf, "¡» «¢«¢¡I%s (%s) ³Q½ð¥X¥h¤F", chatid, user->userid);
		break;
	}
...
chat_unignore(ChatUser *cu, char *msg)
{
	...
	sprintf(chatbuf, (list_delete(&(cu->ignore), ignoree)) ?
		"¡» [%s] ¤£¦A³Q§A§N¸¨¤F" :
		"¡» ±z¨Ã¥¼©¿²¤ [%s]¡A½Ð¥Î /ignore ÀË¬d¦Cªí¡C", ignoree);
    }
    else
    {
	strcpy(chatbuf, "¡» ½Ð«ü©ú¨Ï¥ÎªÌ ID");
    }
    send_to_user(cu, chatbuf, 0, MSG_MESSAGE);
}
...
chat_unban(ChatUser *cu, char *msg)
{
	...
	sprintf(chatbuf, (list_delete(list, unban)) ?
		"¡» [%s] ¤£¦A³Q¦C¬°¶Â¦W³æ" :
		"¡» [%s] ¨Ã¤£¦b¶Â¦W³æ¤¤¡A½Ð¥Î /ban ÀË¬d¦Cªí", unban);
    }
    else
    {
	strcpy(chatbuf, "¡» ½Ð«ü©ú¨Ï¥ÎªÌ ID");
...
chat_makeop(ChatUser *cu, char *msg)
{
    ...
    if (cu == xuser)
    {
	sprintf(chatbuf, "¡° ±z¤w¸g¬OºÞ²z­û(Op)¤F¡AµLªk§ïÅÜ¦Û¤vªºÅv­­");
	send_to_user(cu, chatbuf, 0, MSG_MESSAGE);
	return;
    }
    ...
    {
	// °Ê¤£¤F¥L
	sprintf(chatbuf, "¡° µLªk§ïÅÜ¹ï¤èªººÞ²z­û(Op)Åv­­¡C");
	send_to_user(cu, chatbuf, 0, MSG_MESSAGE);
	return;
    }

    sprintf(chatbuf, "¡° %s %s %s ªººÞ²z­û(Op)Åv­­",
	    cu->chatid, 
