...
func main() {
	...
	fmt.Printf("[aloha.svc] Performance settings: GOMAXPROCS=%d, MaxThreads=%d, GCPercent=%d\n", runtime.GOMAXPROCS(0), *maxThreads, *gcPercent)
	...
	if *debugMode {
		...
		if err := os.MkdirAll(filepath.Dir(*logPath), 0755); err == nil {
			...
			if err == nil {
				...
				log.Printf("[aloha.svc] Performance settings: GOMAXPROCS=%d, MaxThreads=%d, GCPercent=%d", runtime.GOMAXPROCS(0), *maxThreads, *gcPercent)
			} else {
