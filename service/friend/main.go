...
func main() {
	...
	if *daemonize {
		if daemon.IsSocketOccupied(socketPath) {
			fmt.Fprintf(os.Stderr, "[friend.svc] Error: UNIX domain socket %s is already occupied by another instance\n", socketPath)
			os.Exit(1)
		}

		if err := forkDaemon(); err != nil {
			fmt.Fprintf(os.Stderr, "[friend.svc] Failed to daemonize: %v\n", err)
			os.Exit(1)
		}
		fmt.Printf("[friend.svc] Starting PTT BBS Friend Service in background...\n")
		os.Exit(0)
	}

	if *logPath == "" {
		*logPath = filepath.Join(bbsHome, "log", "friend.svc.log")
	}
	...
	if *debugMode {
		...
		log.Printf("[friend.svc] Debug mode enabled (-D/-debug): logging directly to stdout (Verbose=%d)", int(verbose))
	} else {
	...
	if err != nil {
		log.Fatalf("[friend.svc] Failed to initialize Friend Service: %v", err)
	}
