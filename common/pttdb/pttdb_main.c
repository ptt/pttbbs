...
create_main_from_fd(aidu_t aid, char *board, char *title, char *poster, char *ip, char *origin, char *web_link, int len, int fd_content, UUID main_id, UUID content_id, time64_t create_milli_timestamp)
{
    ...
    if(!error_code) {
        error_code = serialize_main_bson(&main_header, &main_bson);
    }
...
read_main_header(UUID main_id, MainHeader *main_header)
{
    ...
    if(!error_code) {
        error_code = deserialize_main_bson(db_result, main_header);
    }
 ...
 */
Err
serialize_main_bson(MainHeader *main_header, bson_t **main_bson)
{
    *main_bson = BCON_NEW(
 ...
 */
Err
deserialize_main_bson(bson_t *main_bson, MainHeader *main_header)
{    
    Err error_code;
 ...
 */
Err
serialize_update_main_bson(UUID content_id, char *updater, char *update_ip, time64_t update_milli_timestamp, int n_total_line, int n_total_block, int len_total, bson_t **main_bson)
{
    *main_bson = BCON_NEW(
