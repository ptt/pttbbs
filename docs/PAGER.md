...
| **`VKEY_HOOK_PRIO_MODAL` (1)** | `pager_modal_key_hook()` | [`mbbsd/pager.c`](file:///usr/local/google/home/hungte/pttdev/pttbbs/mbbsd/pager.c#L845) | **Modal 視窗/歷史面板導覽**：<br>• 僅在彈出水球歷史面板 (`watermode > 0`) 狀態下截獲 `Tab`, `Shift-Tab`, `Ctrl-F`, `Ctrl-G` 等面板導覽按鍵。 |
