...
user_login(void)
{
	...
	if (get_fav_root() != NULL) {
		...
		vans(TEMPFORMAT(STRLEN, "找到 %d 個新看板，確定要加入我的最愛嗎？[y/N]", num)) != 'y') {
		fav_free();
		fav_load();
	    }
	}
...
do_aloha(void)
{
    ...
    friend_svc_login(currutmp->userid, currutmp->pid, get_utmp_id(currutmp));
}
