...
readtitle()
{
    ...
    prints("[¡ö]Â÷¶} [¡÷]¾\\Åª [^P]µoªí¤å³¹ [b]³Æ§Ñ¿ý [d]§R°£ [z]ºëµØ°Ï "
	   "[TAB]¤åºK [h]elp\n\033[7m  ½s¸¹    ¤é ´Á §@  ªÌ       ¤å  ³¹  ¼Ð  ÃD"
	   "                      ¤H®ð:%-5d   \033[m", SHM->bcache[currbid - 1].nuser);
}
...
do_general(int isbid)
{
    ...
    char            genbuf[200], *owner, ctype[8][5] = {"°ÝÃD", "«ØÄ³", "°Q½×", "¤ß±o", "¶¢²á", "½Ð¯q", "¤½§i", "±¡³ø"};
    boardheader_t  *bp;
	...
	outmsg("¹ï¤£°_¡A±z¥Ø«eµLªk¦b¦¹µoªí¤å³¹¡I");
	pressanykey();
	return READ_REDRAW;
    }
#ifdef NO_WATER_POST
	...
	if (water_counts >= 5) {
	    outmsg("¹ï¤£°_¡A±zªº¤å³¹¤Ó¤ôÅo¡A«Ý·|¦Apost§a¡I¤p¯µ³Z:¥i¥Î'X'±ÀÂË¤å³¹");
	    pressanykey();
	    return READ_REDRAW;
	}
    } else {
...
do_bid(int ent, fileheader_t * fhdr, boardheader_t  *bp, char *direct,  struct tm      *ptime)
{
	...
	if( bidinfo.userid[0]) {
	    ...
	    prints("®¥³ß %s ¥H %d ±o¼Ð!", bidinfo.userid, 
		    bidinfo.high);
#ifdef ASSESS
    ...
    {
	vmsg("¨ú®ø¤U¼Ð©Î¼Ðª÷¤£¨¬·m¼Ð");
        return FULLUPDATE;
    }
    
    snprintf(genbuf, sizeof(genbuf),
 "\033[1;31m¡÷ \033[33m%s\033[m\033[33m:%s\033[m%*s %s%-15d¼Ð%15s %02d/%02d\n",
	     cuser->userid,say,
	...
	bidinfo.usermax=mymax;
        ...
        snprintf(genbuf, sizeof(genbuf),
"\033[1;31m¡÷ \033[33m¦Û°ÊÄv¼Ð%s³Ó¥X\033[m\033[33m\033[m%*s%s%-15d¼Ð %02d/%02d\n",
	     cuser->userid, 
...
#define SIZE	sizeof(badpost_reason) / sizeof(char *)
		...
		if(genbuf[0]=='y') {
		    ...
		    prints("%d.%s", i + 1, "¨ä¥L");
		    getdata(b_lines - 1, 0, "½Ð¿ï¾Ü", reason, sizeof(reason), LCECHO);
		    i = reason[0] - '0';
		    if (i <= 0 || i > SIZE)
			getdata(b_lines, 0, "½Ð¿é¤J­ì¦]", reason, sizeof(reason), DOECHO);
		    else
			strcpy(reason, badpost_reason[i - 1]);
		    if (!(inc_badpost(num, 1) % 10)){
			post_violatelaw(xuser.userid, "Ptt ¨t²ÎÄµ¹î", "¦H¤å²Ö­p¤Q½g", "»@³æ¤@±i");
			mail_violatelaw(xuser.userid, "Ptt ¨t²ÎÄµ¹î", "¦H¤å²Ö­p¤Q½g", "»@³æ¤@±i");
			xuser.userlevel |= PERM_VIOLATELAW;
		    }
		    sprintf(genbuf,"¦H¤å°h¦^(%s):%-40.40s", reason, fhdr->title);
...
static char    *board_help[] = {
    "\0¥þ¥\\¯à¬ÝªO¾Þ§@»¡©ú",
    "\01°ò¥»©R¥O",
    "(p)(¡ô)   ¤W²¾¤@½g¤å³¹         (^P/^O/^V)µoªí¤å³¹/Äv¼Ð/¬¡°Ê³s¸p",
    "(n)(¡õ)   ¤U²¾¤@½g¤å³¹         (d)       §R°£¤å³¹",
    ...
    "(V/R)     §ë²¼/¬d¸ß§ë²¼µ²ªG    (^W)(X)   §Ú¦b­þ¸Ì/±ÀÂË¤å³¹/°Ñ»PÄv¼Ð",
    "(x)(w)    Âà¿ý¤å³¹/¥á¤ô²y      (=)/([]<>-+) §ä´M­º½g¤å³¹/¥DÃD¦¡¾\\Åª",
#ifdef INTERNET_EMAIL
    "(F)       ¤å³¹±H¦^Internet¶l½c (U)       ±N¤å³¹ uuencode «á±H¦^¶l½c",
#endif
    "(E)/(Q)   ­«½s¤å³¹/¬d¸ß»ù®æ/°Î¦W(^H)      ¦C¥X©Ò¦³ªº¥D­n¼ÐÃD",
    "\01ªO¥D©R¥O",
    "(M/o)     Á|¦æ§ë²¼/½s¨p§ë²¼¦W³æ (m/c/g)  «O¯d¤å³¹/¿ï¿ýºëµØ/¤åºK",
    "(D)       §R°£¤@¬q½d³òªº¤å³¹    (T/B)    ­«½s¤å³¹¼ÐÃD/­«½s¬ÝªO¼ÐÃD",
    "(I)       ¶}©ñ/¸T¤î¬ÝªO±ÀÂË     (t/^D)   ¼Ð°O¤å³¹/¬å°£¼Ð°Oªº¤å³¹",
    "(O)/(i)   µoªíª`·N¨Æ¶µ ¤å³¹Ãþ§O (H)/(Y)  ¬ÝªOÁôÂÃ/²{¨­ ¨ú®ø±ÀÂË¤å³¹",
#ifdef NO_GAMBLE
    "(W/K/v)    ½s¶iªOµe­±/¤ô±í¦W³æ/¥i¨£¦W³æ",
#else
    "(^G)      Á|¿ì½ä½L/°±¤î¤Uª`/¶}¼ú(W/K/v) ½s¶iªOµe­±/¤ô±í¦W³æ/¥i¨£¦W³æ",
#endif
    NULL
};
...
b_changerecommend(int ent, fileheader_t * fhdr, char *direct)
{
    ...
    substitute_record(fn_board, bp, sizeof(boardheader_t), currbid);
    vmsg("¥»ªO²{¦b %s ±ÀÂË",
         (bp->brdattr & BRD_NORECOMMEND) ? "¸T¤î" : "¶}©ñ");
    return FULLUPDATE;
}
