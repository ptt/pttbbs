...
a_loadname(menu_t * pm)
{
    ...
    int rows = (t_columns < 70) ? (t_lines - 3) / 2 : (t_lines - 3);
    if (rows != pm->header_size) {
	pm->header_size = rows;
...
a_searchtitle(menu_t * pm, int rev)
{
	...
	if (pos == pm->num)
	    ...
	    pos = pm->num - 1;
	if (pos < pm->page || pos >= pm->page + pm->header_size) {
	    pm->page = pos - pos % pm->header_size;

	    if (!a_loadname(pm))
		return pm->now;
	}
	if (mbs_strcasestr(pm->header[pos - pm->page].title, search_str))
	    return pos;
    } while (pos != pm->now);
    return pm->now;
}
...
a_additem(menu_t * pm, const fileheader_t * myheader)
{
    ...
    if (pm->now < pm->page || pm->now >= pm->page + pm->header_size) {
	pm->page = pm->now - (pm->now % pm->header_size);
    }
    /* Ptt */
...
a_newitem(menu_t * pm, int mode)
{
    ...
    if (!getdata(b_lines - 1, 1, mesg[mode], item.title + MACROSTRLEN("◇ "), 55, DOECHO)) {
	if (mode == ADDGROUP)
	    rmdir(fpath);
	else
	    unlink(fpath);
...
a_pasteitem(menu_t * pm, int mode)
{
	...
	if (ans[0] == 'y') {
	    ...
	    title = mbs_nth(cq->copytitle, 1);
	    snprintf(item.title, sizeof(item.title), "%s%s", prefix, title ? title : "");
	    a_additem(pm, &item);
	    cq->copyfile[0] = '\0';
	}
    }
...
a_delete(menu_t * pm, const char *backup_dir)
{
	...
	const char *title = mbs_nth(fhdr->title, 1);
	snprintf(backup.title, sizeof(backup.title), "◆%s", title ? title : "");
...
a_newtitle(const menu_t * pm)
{
    ...
    char *title = (char *)mbs_nth(item.title, 2);
    if (!title)
	title = item.title + strlen(item.title);
    STRLCPY(buf, title);
    if (getdata_buf(b_lines - 1, 0, "   新標題: ", buf, 60, DOECHO)) {
	strlcpy(title, buf, sizeof(item.title) - (title - item.title));
	setadir(buf, pm->path);
...
static void
announce_sync_pos(cmd_ctx_t *ctx)
{
    ...
    if (dashf(fname) && cx->me->level >= MANAGER) {
        ...
        *quote_file = 0;
        ...
        if (vedit2(fname, NA, NULL, edflags) != -1) {
            ...
            modify_fileheader(fpath, cx->me->header + cx->me->now - cx->me->page, cx->me->now + 1);
        }
        ctx->reload = true;
    }
    return 0;
