...
reverse_friend_stat(int stat)
{
    ...
    if (stat & IBH)
	stat1 |= IBH;
    return friend_normalize_stat(stat1);
}

/* Previous friend_online[] of currutmp, used to keep "he -> me" relations
 * (HFM/HRM/HSM) while reloading in local fallback mode: the peers' entries
 * for us were just cleared, so they can't be read back from their arrays. */
static const unsigned int *friend_bit_hint;
static int friend_bit_nhint;

void
set_friend_bit_hint(const unsigned int *entries, int count)
{
    friend_bit_hint = entries;
    friend_bit_nhint = entries ? count : 0;
}
...
set_friend_bit(const userinfo_t * me, const userinfo_t * ui)
{
    ...
    if (!me || !ui || !me->uid || !ui->uid)
	return 0;

    if (me == currutmp) {
	if (is_local_friend(ui->uid))
	    hit |= IFH;
	if (is_local_reject(ui->uid))
	    hit |= IRH;
    }

    int found = 0;
    me_slot = get_utmp_id(me);
    if (VALID_USHM_ENTRY(me_slot)) {
	for (i = 0; i < MAX_FRIEND_ONLINE && ui->friend_online[i]; i++) {
	    unsigned int entry = ui->friend_online[i];
	    if (FRIEND_ONLINE_SLOT(entry) == me_slot &&
		FRIEND_ONLINE_VALID_UID(entry, me->uid)) {
		hit |= reverse_friend_stat(FRIEND_ONLINE_STAT(entry)) & (HFM | HRM | HSM);
		found = 1;
		break;
	    }
	}
    }
    if (!found && me == currutmp && friend_bit_hint) {
	int ui_slot = get_utmp_id(ui);
	for (i = 0; i < friend_bit_nhint; i++) {
	    unsigned int entry = friend_bit_hint[i];
	    if (FRIEND_ONLINE_SLOT(entry) == ui_slot &&
		FRIEND_ONLINE_VALID_UID(entry, ui->uid)) {
		hit |= FRIEND_ONLINE_STAT(entry) & (HFM | HRM | HSM);
		break;
	    }
	}
    }
    return friend_normalize_stat(hit);
}
...
void login_friend_online(int do_login GCC_UNUSED)
{
    ...
    /* Peers may be legacy binaries before the cutoff (reject[] follows
     * friend_online[MAX_FRIEND]); keep their arrays within MAX_FRIEND. */
    int             peer_limit = (now < FRIEND_LEGACY_COMPAT_CUTOFF) ?
				 MAX_FRIEND : MAX_FRIEND_ONLINE;

    currutmp->friend_svc_flag = 0;
    if (currutmp->friendtotal < 0)
	currutmp->friendtotal = 0;
    for (i = 0; i < SHM->UTMPnumber && currutmp->friendtotal < MAX_FRIEND_ONLINE; i++) {
	...
	if (uentp && uentp->uid && (stat = set_friend_bit(currutmp, uentp))) {
	    ...
	    if (currutmp->friendtotal >= 0 && currutmp->friendtotal < MAX_FRIEND_ONLINE) {
		int tag_uid = (now >= FRIEND_LEGACY_COMPAT_CUTOFF) ? uentp->uid : 0;
		currutmp->friend_online[currutmp->friendtotal++] =
		    FRIEND_ONLINE_PACK(stat, tag_uid, get_utmp_id(uentp));
	    }
	    if (uentp != currutmp && uentp->friendtotal >= 0 && uentp->friendtotal < peer_limit) {
		int my_tag_uid = (now >= FRIEND_LEGACY_COMPAT_CUTOFF) ? currutmp->uid : 0;
		unsigned int packed = FRIEND_ONLINE_PACK(stat1, my_tag_uid, offset);
		int ftotal = uentp->friendtotal;
		int k, ins = ftotal;
		for (k = 0; k < ftotal && k < MAX_FRIEND_ONLINE; k++) {
		    int slot_k = FRIEND_ONLINE_SLOT(uentp->friend_online[k]);
		    if (slot_k == offset) {
			uentp->friend_online[k] = packed;
			ins = -1;
			break;
		    }
		    if (ins == ftotal && slot_k > offset)
			ins = k;
		}
		if (ins >= 0 && ftotal < peer_limit) {
		    if (ins < ftotal) {
			memmove(&uentp->friend_online[ins + 1], &uentp->friend_online[ins],
				sizeof(unsigned int) * (size_t)(ftotal - ins));
		    }
		    uentp->friend_online[ins] = packed;
		    uentp->friendtotal = ftotal + 1;
		}
	    }
	}
    }
		...
		int *myfriend, int *friendme, int *badfriend)
{
    ...
    *myfriend = *friendme = 1;
    ...
    friends[ngets].uoffset = get_utmp_id(currutmp);
