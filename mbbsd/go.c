...
GO_result(struct GOData *gd)
{
    ...
    {
	move(5, 46);
	prints("%s ¤è¥Ø¼Æ¡G%-3.1f       ", bw_chess[gd->me - 1], count[0]);
	move(6, 46);
	prints("%s ¤è¥Ø¼Æ¡G%-3.1f       ", bw_chess[gd->he - 1], count[1]);
	move(21, 46);
	clrtoeol();
	move(8, 46);
    ...
    {
	move(5, 46);
	prints("%s ¤è¥Ø¼Æ¡G%-3.1f       ", bw_chess[gd->me - 1], count[1]);
	move(6, 46);
	prints("%s ¤è¥Ø¼Æ¡G%-3.1f       ", bw_chess[gd->he - 1], count[0]);
	move(21, 46);
	clrtoeol();
	move(8, 46);
...
gochess(int fd)
{
    ...
    */
	...
	if(scr_need_redraw){
	    move(2, 46);
	    prints("%s ¤è´£¤l¼Æ¡G%3d", bw_chess[gd.me - 1], gd.hk);
	    move(3, 46);
	    prints("%s ¤è´£¤l¼Æ¡G%3d", bw_chess[gd.he - 1], gd.mk);
		...
		prints("½ü¨ì¦Û¤v¤U¤F.... §Ú¬O %s", bw_chess[gd.me - 1]);
	    else
		outs("µ¥«Ý¹ï¤è¤U¤l....");
		...
		if (pool[totalgo - 1].x == -1 || pool[totalgo - 1].y == -1)
		    prints("%s #%-3d PASS ¤W¤@¤â    ", gd.win ? bw_chess[(totalgo - 1) & 1] : bw_chess[totalgo & 1], totalgo);
		else
		    prints("%s #%-3d %.1s%-2d  ¤W¤@¤â    ", gd.win ? bw_chess[(totalgo - 1) & 1] : bw_chess[totalgo & 1], totalgo, locE + pool[totalgo - 1].x, pool[totalgo - 1].y + 1);
	    }
...
GoBot(void)
{
	...
	if(scr_need_redraw){
	    move(8, 46);
	    clrtoeol();
	    prints("½ü¨ì %s ¤è¤U¤F....", bw_chess[gd.me - 1]);
