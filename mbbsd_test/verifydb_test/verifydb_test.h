...
// verifydb-test
class VerifydbTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    // load board
    check_SHM();
    fprintf(stderr, "[verifydb_test.SetUp]: to reload_bcache\n");
    reload_bcache();
    fprintf(stderr, "[verifydb_test.SetUp]: after reload_bcache\n");
    ...
    for(int i = 0; i < argc; i++) {
      fprintf(stderr, "[verifydb_test.SetUp] (%d/%d) argv: %s\n", i, argc, argv[i]);
    }

    ProgramOption opt = {};
    fprintf(stderr, "[verifydb_test.SetUp] to parse_argv\n");
    parse_argv(argc, argv, &opt);
    fprintf(stderr, "[verifydb_test.SetUp] after parse_argv\n");
    ...
    vkey_purge();
    fprintf(stderr, "[verifydb_test.SetUp]: after load_current_user\n");
    sprintf(buf, "y");
    put_vin((unsigned char *)buf, strlen(buf));
    user_login();
    vkey_purge();
